public class simulator {
   ...
   {
for(Team team:team){
    int wonGames=matches.stream().filter(m->m.getWinner().equals(teams)).collect(collectors.toList()).size();
    int lostGames=matches.stream().filter(m->m.getloser().equals(teams)).collect(collectors.toList()).size();
